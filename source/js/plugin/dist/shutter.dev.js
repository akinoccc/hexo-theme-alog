...
(function ($, window, document, undefined) {
  var Shutter = function Shutter(elem, options) {
    this.defaults = {
      shutterW: 1200,
      shutterH: 500,
      isAutoPlay: false,
  ...
  Shutter.prototype = {
    ...
    moveSwitch: function moveSwitch(randomNum, command, index) {
      switch (randomNum) {
        case 0:
    ...
    toggleMove: function toggleMove(command, index) {
      if (!command) {
    ...
    initalShutter: function initalShutter() {
      ...
      this.$shutterDesc.find('p').text(this.shutterTitle);
      this.$shutter.css({
    ...
    inital: function inital(elem) {
      ...
      this.b_stop = true;
      this.shutterTitle = this.$shutterItem.eq(0).attr('data-shutter-title');
      this.playTime = null;
      this.initalShutter();
      ...
      window.onresize = function () {
        var $imgPlayer = $('.img-player');
        _this.shutterW = $imgPlayer.width();
        _this.shutterH = $imgPlayer.height();

        _this.$shutter.css({
          width: _this.shutterW,
          height: _this.shutterH
        });
      };
    ...
    // 图片切换方法
    recovery: function recovery(target, cur, index, backup, interval) {
      var self = this;
      setTimeout(function () {
    ...
    gridWhole: function gridWhole(index, showNum) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
    ...
    gridTop: function gridTop(index, showNum) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
    ...
    gridLeft: function gridLeft(index, showNum) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
    ...
    gridOpacity: function gridOpacity(index) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
    ...
    gridAccordion: function gridAccordion(index) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
    ...
    gridLittle: function gridLittle(index) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
    ...
    gridSwitch: function gridSwitch(index) {
      var self = this;
      var $curElem = this.$shutterItem.eq(this.curDisplay);
      var $targetElem = this.$shutterItem.eq(index);
      var backup = $curElem.html();
